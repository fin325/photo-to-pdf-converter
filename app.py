...
st.markdown("""
    ...
    /* Текст кнопки, когда ФАЙЛЫ ЗАГРУЖЕНЫ (чтобы не дублировалось) */
    ...
    /* Кнопка "Скачать PDF" (делаем её зеленоватой для отличия) */
    .stDownloadButton>button {
        background: rgba(74, 226, 133, 0.4) !important;
        color: white !important;
        border-radius: 12px !important;
        border: 1px solid rgba(255, 255, 255, 0.2) !important;
        width: 100% !important;
...
if uploaded_files:
    ...
    if convert_clicked:
        with st.spinner('Магия конвертации...'):
        ...
        st.success("Готово! Ваш PDF создан.")
        ...
        pdf_display = f'''
            <iframe src="data:application/pdf;base64,{b64}" 
                    ...
                    style="border-radius:15px; border:none; margin-top:20px;">
            </iframe>'''
        st.markdown(pdf_display, unsafe_allow_html=True)

    # 👁 ПРЕДПРОСМОТР ИЗОБРАЖЕНИЙ (внизу)
    st.markdown("---")
    st.write("🖼 Предпросмотр загруженных фото:")
    cols = st.columns(4)
